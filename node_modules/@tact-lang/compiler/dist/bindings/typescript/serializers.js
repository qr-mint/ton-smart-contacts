...
const intSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.loadIntBig(${v.bits}) : null;`);
        }
        else {
...
const uintSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.loadUintBig(${v.bits}) : null;`);
        }
        else {
...
const coinsSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.loadCoins() : null;`);
        }
        else {
...
const varIntSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        const loader = v.format === "varint16" || v.format === "varint32"
            ? "loadVarIntBig"
            : "loadVarUintBig";
        const length = v.format === "varint16" || v.format === "varuint16" ? 2 : 4;
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.${loader}(${length}) : null;`);
        }
        else {
    ...
    tsStore(v, builder, field, w) {
        const storer = v.format === "varint16" || v.format === "varint32"
            ? "storeVarInt"
            : "storeVarUint";
        const length = v.format === "varint16" || v.format === "varuint16" ? 2 : 4;
        if (v.optional) {
            w.append(`if (${field} !== null && ${field} !== undefined) { ${builder}.storeBit(true).${storer}(${field}, ${length}); } else { ${builder}.storeBit(false); }`);
        }
        else {
...
const boolSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.loadBit() : null;`);
        }
        else {
...
const cellSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.loadRef()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""} : null;`);
        }
        else {
            w.append(`const ${field} = ${slice}.loadRef()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
        }
    },
    tsLoadTuple(v, reader, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${reader}.readCellOpt()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
        }
        else {
            w.append(`const ${field} = ${reader}.readCell()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
        }
    },
    tsStore(v, builder, field, w) {
        if (v.optional) {
            w.append(`if (${field} !== null && ${field} !== undefined) { ${builder}.storeBit(true).storeRef(${field}${v.kind !== "cell" ? ".asCell()" : ""}); } else { ${builder}.storeBit(false); }`);
        }
        else {
            w.append(`${builder}.storeRef(${field}${v.kind !== "cell" ? ".asCell()" : ""});`);
        }
    },
    tsStoreTuple(v, to, field, w) {
        if (v.optional) {
            w.append(`${to}.write${getCellLikeTsType(v)}(${field}${v.kind !== "cell" ? "?.asCell()" : ""});`);
        }
        else {
            w.append(`${to}.write${getCellLikeTsType(v)}(${field}${v.kind !== "cell" ? ".asCell()" : ""});`);
        }
...
const remainderSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        w.append(`const ${field} = ${slice}${v.kind !== "slice" ? getCellLikeTsAsMethod(v) : ""};`);
    },
    tsLoadTuple(v, reader, field, w) {
        w.append(`const ${field} = ${reader}.readCell()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
    },
    tsStore(v, builder, field, w) {
        w.append(`${builder}.storeBuilder(${field}${v.kind !== "builder" ? ".asBuilder()" : ""});`);
    },
    tsStoreTuple(v, to, field, w) {
        w.append(`${to}.write${getCellLikeTsType(v)}(${field}${v.kind !== "cell" ? ".asCell()" : ""});`);
    },
...
const fixedBytesSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.loadBuffer(${v.bytes}) : null;`);
        }
        else {
...
const stringSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? ${slice}.loadStringRefTail() : null;`);
        }
        else {
    ...
    abiMatcher(src) {
        if (src.kind === "simple") {
            if (primitiveTypes.includes(src.type)) {
                (0, errors_1.throwInternalCompilerError)(`Unable to resolve serializer for ${src.type} with ${src.format ?? null} format`);
            }
        }
        return null;
    },
    tsType(_v) {
        (0, errors_1.throwInternalCompilerError)("Unreachable");
    },
...
const struct = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`const ${field} = ${slice}.loadBit() ? load${v.name}(${slice}) : null;`);
        }
        else {
    ...
    tsLoadTuple(v, reader, field, w, fromGet) {
        if (v.optional) {
            ...
            if (fromGet) {
                w.append(`const ${field} = loadGetterTuple${v.name}(${reader});`);
            }
            else {
...
const map = {
    ...
    tsLoad(v, slice, field, w) {
        w.append(`const ${field} = Dictionary.load(${getKeyParser(v.key)}, ${getValueParser(v.value)}, ${slice});`);
    },
    tsLoadTuple(v, reader, field, w) {
        w.append(`const ${field} = Dictionary.loadDirect(${getKeyParser(v.key)}, ${getValueParser(v.value)}, ${reader}.readCellOpt());`);
    },
    tsStore(v, builder, field, w) {
