...
function writeTypescript(abi, ctx, constants, contract, init) {
    ...
    if (init) {
        ...
        w.inIndent(() => {
            // Code references
            const code = core_1.Cell.fromBase64(init.code).toBoc().toString("hex");
            ...
            if (init.system !== null && !(0, features_1.enabledOptimizedChildCode)(ctx)) {
                const system = core_1.Cell.fromBase64(init.system)
                    .toBoc()
                    .toString("hex");
    ...
    w.inIndent(() => {
        ...
        w.append(`public static readonly errors = ${abi.name}_errors_backward;`);
        ...
        w.append(`};`);
        w.append();
        w.append(`constructor(address: Address, init?: { code: Cell, data: Cell }) {`);
        w.inIndent(() => {
        ...
        if (abi.getters) {
            for (const g of abi.getters) {
                ...
                w.inIndent(() => {
                    ...
                    const explicitMethodId = method?.ast.attributes.find((attr) => attr.type === "get")?.methodId;
