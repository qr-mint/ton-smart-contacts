...
function writeAll(ctx, wCtx, name, abiLink, contractCodes) {
    ...
    for (const t of sortedTypes) {
        if (t.kind === "contract" || t.kind === "struct") {
            ...
            (0, writeSerialization_1.writeParser)(t.name, t.kind === "contract", "with-opcode", allocation, wCtx);
            (0, writeSerialization_1.writeParser)(t.name, t.kind === "contract", "no-opcode", allocation, wCtx);
    ...
    for (const t of sortedTypes) {
        if (t.kind === "contract" && t.init) {
            ...
            (0, writeSerialization_1.writeParser)((0, id_1.funcInitIdOf)(t.name), false, "with-opcode", allocation, wCtx);
        }
    }
    // Storage Functions
    for (const t of sortedTypes) {
