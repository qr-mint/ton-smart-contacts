...
function writeStdlib(ctx) {
    ...
    ctx.fun("__tact_dict_replace_ref", () => {
        ctx.signature(`((cell), (int)) __tact_dict_replace_ref(cell dict, int key_len, slice index, cell value)`);
        ctx.context("stdlib");
        ctx.asm("(value index dict key_len)", "DICTREPLACEREF");
    });
    ctx.fun("__tact_dict_replaceget_ref", () => {
        ctx.signature(`((cell), (cell, int)) __tact_dict_replaceget_ref(cell dict, int key_len, slice index, cell value)`);
        ctx.context("stdlib");
        ctx.asm("(value index dict key_len)", "DICTREPLACEGETREF NULLSWAPIFNOT");
    });
    ...
    ctx.fun("__tact_dict_delete_get", () => {
        ctx.signature(`(cell, (slice, int)) __tact_dict_delete_get(cell dict, int key_len, slice index)`);
        ctx.context("stdlib");
        ctx.asm("(index dict key_len)", "DICTDELGET NULLSWAPIFNOT");
    });
    ctx.fun("__tact_dict_delete_get_ref", () => {
        ctx.signature(`(cell, (cell, int)) __tact_dict_delete_get_ref(cell dict, int key_len, slice index)`);
        ctx.context("stdlib");
    ...
    ctx.fun("__tact_debug", () => {
        ctx.signature(`forall X -> () __tact_debug(X value, slice debug_print_1, slice debug_print_2)`);
        ctx.flag("impure");
        ctx.context("stdlib");
        ctx.asm("", "STRDUMP DROP STRDUMP DROP s0 DUMP DROP");
    });
    ctx.fun("__tact_debug_str", () => {
        ctx.signature(`() __tact_debug_str(slice value, slice debug_print_1, slice debug_print_2)`);
        ctx.flag("impure");
        ctx.context("stdlib");
        ctx.asm("", "STRDUMP DROP STRDUMP DROP STRDUMP DROP");
    });
    ctx.fun("__tact_debug_bool", () => {
        ctx.signature(`() __tact_debug_bool(int value, slice debug_print_1, slice debug_print_2)`);
        ctx.flag("impure");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
    ...
    ctx.fun("__tact_debug_address", () => {
        ctx.signature(`() __tact_debug_address(slice address, slice debug_print_1, slice debug_print_2)`);
        ctx.flag("impure");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
    ...
    ctx.fun("__tact_debug_stack", () => {
        ctx.signature(`() __tact_debug_stack(slice debug_print_1, slice debug_print_2)`);
        ctx.flag("impure");
        ctx.context("stdlib");
    ...
    for (let i = 1; i <= writeStruct_1.maxTupleSize; i++) {
...
function getSignatureKeyType(key) {
    return key === "uint" ? "int" : key;
}
function getSignatureValueType(value) {
    return value === "slice" || value === "cell" ? value : "int";
}
function genTactDictGet(ctx, key, value) {
    ...
    ctx.fun(`__tact_dict_get_${key}_${value}`, () => {
        ctx.signature(`${signatureValueType} __tact_dict_get_${key}_${value}(cell d, int kl, ${signatureKeyType} k${valBitsArg()})`);
        ctx.flag("inline");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
...
function genTactDictExists(ctx, key) {
    const signatureKeyType = getSignatureKeyType(key);
    const dictGet = () => {
        switch (key) {
            case "slice":
    ...
    ctx.fun(`__tact_dict_exists_${key}`, () => {
        ctx.signature(`int __tact_dict_exists_${key}(cell d, int kl, ${signatureKeyType} k)`);
        ctx.flag("inline");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
...
function genTactDictSet(ctx, key, value) {
    const signatureKeyType = getSignatureKeyType(key);
    const signatureValueType = getSignatureValueType(value);
    const valBitsArg = () => {
        switch (value) {
    ...
    // prettier-ignore
    const returnExpr = () => (0, tricks_1.match)(key, value)
    ...
    ctx.fun(`__tact_dict_set_${key}_${value}`, () => {
        ctx.signature(`(cell, ()) __tact_dict_set_${key}_${value}(cell d, int kl, ${signatureKeyType} k, ${signatureValueType} v${valBitsArg()})`);
        ctx.flag("inline");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
...
function genTactDictGetMin(ctx, key, value) {
    const signatureKeyType = getSignatureKeyType(key);
    const signatureValueType = getSignatureValueType(value);
    const valBitsArg = () => {
        switch (value) {
    ...
    // prettier-ignore
    const dictGetMin = () => (0, tricks_1.match)(key, value)
    ...
    ctx.fun(`__tact_dict_min_${key}_${value}`, () => {
        ctx.signature(`(${signatureKeyType}, ${signatureValueType}, int) __tact_dict_min_${key}_${value}(cell d, int kl${valBitsArg()})`);
        ctx.flag("inline");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
...
function genTactDictGetNext(ctx, key, value) {
    const signatureKeyType = getSignatureKeyType(key);
    const signatureValueType = getSignatureValueType(value);
    const valBitsArg = () => {
        switch (value) {
    ...
    ctx.fun(`__tact_dict_next_${key}_${value}`, () => {
        ctx.signature(`(${signatureKeyType}, ${signatureValueType}, int) __tact_dict_next_${key}_${value}(cell d, int kl, ${signatureKeyType} pivot${valBitsArg()})`);
        ctx.flag("inline");
        ctx.context("stdlib");
        ctx.body(() => {
...
function genTactDictReplace(ctx, key, value) {
    const signatureKeyType = getSignatureKeyType(key);
    const signatureValueType = getSignatureValueType(value);
    const valBitsArg = () => {
        switch (value) {
    ...
    ctx.fun(`__tact_dict_replace_${key}_${value}`, () => {
        ctx.signature(`(cell, (int)) __tact_dict_replace_${key}_${value}(cell d, int kl, ${signatureKeyType} k, ${signatureValueType} v${valBitsArg()})`);
        ctx.flag("inline");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
...
function genTactDictReplaceGet(ctx, key, value) {
    const signatureKeyType = getSignatureKeyType(key);
    const signatureValueType = getSignatureValueType(value);
    const valBitsArg = () => {
        switch (value) {
    ...
    ctx.fun(`__tact_dict_replaceget_${key}_${value}`, () => {
        ctx.signature(`(cell, (${signatureValueType})) __tact_dict_replaceget_${key}_${value}(cell d, int kl, ${signatureKeyType} k, ${signatureValueType} v${valBitsArg()})`);
        ctx.flag("inline");
        ctx.context("stdlib");
        ctx.body(() => {
            ctx.write(`
